...
export default function MediaAthletesPage() {
  ...
  useEffect(() => {
    ...
    const loadAthletes = async () => {
      try {
        ...
        const payload = (await response.json()) as { athletes?: MediaAthleteDirectoryEntry[]; error?: string };
  ...
  const sports = useMemo(() => [
    "Tous",
    ...Array.from(new Set(normalizedAthletes.map((athlete) => athlete.sport).filter(Boolean)))
      .sort((left, right) => left.localeCompare(right, "fr")),
  ], [normalizedAthletes]);
  const visibleAthletes = useMemo(() => {
    const search = query.trim().toLocaleLowerCase("fr");
    return normalizedAthletes.filter((athlete) => (
      (!search || athlete.name.toLocaleLowerCase("fr").includes(search))
      ...
      {!loading && !error && visibleAthletes.length > 0 ? (
        <section className="partner-athletes-grid" aria-label="Annuaire des athlètes">
          {visibleAthletes.map((athlete) => (
            <Link key={athlete.athleteId} href={`/media/athletes/${encodeURIComponent(athlete.athleteId)}`} className="partner-athlete-card">
