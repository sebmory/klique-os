...
type HandlerDependencies = {
  ...
  confirm: (request: Request, orderId: string) => Promise<ConfirmedAthleteMembershipProspectOrder>;
};
...
const defaultDependencies: HandlerDependencies = {
  ...
  confirm: confirmAthleteMembershipProspectOrderPayment,
};

class AdminAthleteMembershipProspectOrderConfirmationRouteError extends Error {}
...
const validateRequest = async (request: Request, context: RouteContext): Promise<string> => {
  if ([...new URL(request.url).searchParams.keys()].length > 0) {
    throw new AdminAthleteMembershipProspectOrderConfirmationRouteError();
  }
...
const publicOrder = (order: ConfirmedAthleteMembershipProspectOrder["order"]) => ({
  id: order.id,
...
const errorResponse = (error: unknown): NextResponse => {
  if (error instanceof AdminAthleteMembershipProspectOrderConfirmationRouteError) {
...
export const createAdminAthleteMembershipProspectOrderConfirmationHandlers = (
...
const handlers = createAdminAthleteMembershipProspectOrderConfirmationHandlers();
